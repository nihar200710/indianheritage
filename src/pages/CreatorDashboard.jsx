...
export default function CreatorDashboard() {
  ...
  const handleUpload = () => {
    ...
    // Add new post to top of list as "Pending Approval"
    setMyPosts([{
      id: Date.now(),
      title: title,
      category: category,
  ...
  return (
    <div style={{ padding: '40px 20px', maxWidth: '1200px', margin: '0 auto', minHeight: '80vh', position: 'relative' }}>
      ...
      <div style={{ background: 'white', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        ...
        {myPosts.map((post) => (
          <div key={post.id} style={{ padding: '25px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px' }}>
            ...
            {/* Stats (Only show stats if published, otherwise show dashes) */}
      ...
      {/* 🌟 PREMIUM LIQUID GLASS MODAL: UPLOAD CONTENT 🌟 */}
      {isUploading && (
            ...
            {!uploadSuccess ? (
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  <div>
                    <label style={{ display: 'block', fontWeight: 'bold', color: '#475569', marginBottom: '8px' }}>Title</label>
                    <input 
                      type="text" 
                      value={title}
                      onChange={(e) => setTitle(e.target.value)}
                      placeholder="E.g., Architectural Marvels of Konark..." 
                  ...
                  <div>
                    <label style={{ display: 'block', fontWeight: 'bold', color: '#475569', marginBottom: '8px' }}>Content Matter</label>
                    <textarea 
                      value={content}
                      onChange={(e) => setContent(e.target.value)}
                      placeholder="Write your article here..." 
                      style={{ width: '100%', height: '150px', padding: '12px 15px', borderRadius: '8px', border: '1px solid #cbd5e1', outline: 'none', fontSize: '1rem', resize: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
                    ></textarea>
                  </div>

                  <button 
                    onClick={handleUpload}
                    style={{ width: '100%', background: '#8b5cf6', color: 'white', border: 'none', padding: '15px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '1.1rem', marginTop: '10px', boxShadow: '0 4px 10px rgba(139, 92, 246, 0.3)' }}
                  >
                    Submit for Review
                  </button>
                </div>
              ...
              <div style={{ textAlign: 'center', padding: '40px 0' }}>
                ...
                <h2 style={{ margin: '0 0 10px 0', color: '#166534', fontSize: '2rem' }}>Submitted!</h2>
                <p style={{ color: '#475569', fontSize: '1.1rem' }}>Your content has been sent to the Admin for approval.</p>
              </div>
            )}
          </div>
        </div>
